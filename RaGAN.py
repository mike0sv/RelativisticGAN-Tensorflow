...
class RaGAN(object):
    def __init__(self, sess, args):
        ...
        self.model_name = (Ra + self.gan_type.upper()) # name for checkpoint
    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ch = 1024
            x = deconv(z, channels=ch, kernel=4, stride=1, padding='VALID', use_bias=False, scope='deconv')
            ...
            for i in range(self.layer_num) :
                if self.up_sample :
                    ...
                    x = conv(x, channels=ch // 2, kernel=3, stride=1, pad=1, scope='up_conv_' + str(i))
                    x = batch_norm(x, is_training, scope='batch_norm_' + str(i))
                    x = relu(x)

                else :
                    x = deconv(x, channels=ch // 2, kernel=4, stride=2, use_bias=False, scope='deconv_' + str(i))
                    x = batch_norm(x, is_training, scope='batch_norm_' + str(i))
                    x = relu(x)
            ...
            if self.up_sample :
                x = up_sample(x, scale_factor=2)
                x = conv(x, channels=self.c_dim, kernel=3, stride=1, pad=1, scope='G_conv_logit')
                x = tanh(x)

            else :
                x = deconv(x, channels=self.c_dim, kernel=4, stride=2, use_bias=False, scope='G_deconv_logit')
                x = tanh(x)
    ...
    def model_dir(self):
        ...
        return "{}_{}_{}_{}_{}".format(
            self.model_name, self.dataset_name, self.img_size, self.n_critic, self.sn)
